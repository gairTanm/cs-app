...
static void *extend_heap(size_t words) {
    ...
    PUT(HDRP(NEXT_BLCKP(bp)), PACK(size, ALLOCATED));
...
static void *coalesce(void *bp) {
    int prev_allocated = GET_ALLOC(PREV_BLCKP(bp));
    int next_allocated = GET_ALLOC(NEXT_BLCKP(bp));
    ...
    if (!prev_allocated && next_allocated) {
        size_t prev_size = GET_SIZE(HDRP(PREV_BLCKP(bp)));

        PUT(HDRP(PREV_BLCKP(bp)), PACK(prev_size + curr_size, !ALLOCATED));
        PUT(FTRP(bp), PACK(prev_size + curr_size, !ALLOCATED));
    }

    if (prev_allocated && !next_allocated) {
        size_t next_size = GET_SIZE(HDRP(NEXT_BLCKP(bp)));

        PUT(FTRP(NEXT_BLCKP(bp)), PACK(next_size + curr_size, !ALLOCATED));
        PUT(HDRP(bp), PACK(next_size + curr_size, !ALLOCATED));
    }
    ...
    if (!prev_allocated && !next_allocated) {
        ...
        PUT(HDRP(PREV_BLCKP(bp)),
            PACK(next_size + prev_size + curr_size, !ALLOCATED));
        PUT(FTRP(NEXT_BLCKP(bp)),
            PACK(next_size + prev_size + curr_size, !ALLOCATED));
    }
}
